...
'''
This class is breaking down the system when the experiments are running
class OverheadTracer(Tracer):
    DEVICE_STR = '/dev/litmus/ft_cpu_trace'
    ...
    def __init__(self, output_dir):
        ...
        if OverheadTracer.enabled():
            for cpu in range(num_cpus()):
                stdout_f = open('%s/ft.%d.bin' % (self.output_dir, cpu), 'w')
                stderr_f = open('%s/ft.%d.stderr.txt' % (self.output_dir, cpu), 'w')
                dev = '{0}{1}'.format(OverheadTracer.DEVICE_STR, cpu)
                ftc = FTcat(conf.BINS['ftcat'], stdout_f, stderr_f, dev, conf.OVH_ALL_EVENTS, cpu=cpu)
                self.bins.append(ftc)

    @staticmethod
    def enabled():
        return is_device('/dev/litmus/ft_cpu_trace0')
'''

class OverheadTracer1(Tracer):
    DEVICE_STR = '/dev/litmus/ft_cpu_trace1'
    

    def __init__(self, output_dir):
        super(OverheadTracer1, self).__init__("Overhead Trace", output_dir, True)

        stdout_f = open('{0}/overhead1_{1}'.format(self.output_dir, conf.FILES['ft_data']), 'w')
        stderr_f = open('{0}/overhead1_{1}.stderr.txt'.format(self.output_dir, conf.FILES['ft_data']), 'w')
        ftc = FTcat(conf.BINS['ftcat'], stdout_f, stderr_f,
                OverheadTracer1.DEVICE_STR, conf.OVH_ALL_EVENTS)

        self.bins.append(ftc)

    @staticmethod
    def enabled():
        return is_device(OverheadTracer1.DEVICE_STR)

class OverheadTracer0(Tracer):
    DEVICE_STR = '/dev/litmus/ft_cpu_trace0'
    

    def __init__(self, output_dir):
        super(OverheadTracer0, self).__init__("Overhead Trace", output_dir, True)
        ...
        ftc = FTcat(conf.BINS['ftcat'], stdout_f, stderr_f,
                OverheadTracer0.DEVICE_STR, conf.OVH_ALL_EVENTS)

        self.bins.append(ftc)

    @staticmethod
    def enabled():
        return is_device(OverheadTracer0.DEVICE_STR)

class OverheadTracer2(Tracer):
    DEVICE_STR = '/dev/litmus/ft_cpu_trace2'
    

    def __init__(self, output_dir):
        super(OverheadTracer2, self).__init__("Overhead Trace", output_dir, True)

        stdout_f = open('{0}/overhead2_{1}'.format(self.output_dir, conf.FILES['ft_data']), 'w')
        stderr_f = open('{0}/overhead2_{1}.stderr.txt'.format(self.output_dir, conf.FILES['ft_data']), 'w')
        ftc = FTcat(conf.BINS['ftcat'], stdout_f, stderr_f,
                OverheadTracer2.DEVICE_STR, conf.OVH_ALL_EVENTS)

        self.bins.append(ftc)

    @staticmethod
    def enabled():
        return is_device(OverheadTracer2.DEVICE_STR)

class OverheadTracer3(Tracer):
    DEVICE_STR = '/dev/litmus/ft_cpu_trace3'
    

    def __init__(self, output_dir):
        super(OverheadTracer3, self).__init__("Overhead Trace", output_dir, True)

        stdout_f = open('{0}/overhead3_{1}'.format(self.output_dir, conf.FILES['ft_data']), 'w')
        stderr_f = open('{0}/overhead3_{1}.stderr.txt'.format(self.output_dir, conf.FILES['ft_data']), 'w')
        ftc = FTcat(conf.BINS['ftcat'], stdout_f, stderr_f,
                OverheadTracer3.DEVICE_STR, conf.OVH_ALL_EVENTS)

        self.bins.append(ftc)

    @staticmethod
    def enabled():
        return is_device(OverheadTracer3.DEVICE_STR)
...
register_tracer(OverheadTracer0, "overhead0")
#register_tracer(OverheadTracer1, "overhead1")
#register_tracer(OverheadTracer2, "overhead2")
#register_tracer(OverheadTracer3, "overhead3")
